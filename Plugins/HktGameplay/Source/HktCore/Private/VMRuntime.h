...
/**
 * FVMRuntime - 단일 VM의 실행 상태
 */
struct HKTSIMULATION_API FVMRuntime
{
    /** 실행 중인 프로그램 (공유, 불변) */
    const FVMProgram* Program = nullptr;
    
    /** 로컬 데이터 스토어 */
    struct FVMStore* Store = nullptr;
...
// ============================================================================
// FVMRuntimePool - SOA 레이아웃의 런타임 풀
// ============================================================================

class HKTSIMULATION_API FVMRuntimePool
{
public:
    FVMRuntimePool();
    
    FVMHandle Allocate();
    void Free(FVMHandle Handle);
    
    FVMRuntime* Get(FVMHandle Handle);
    const FVMRuntime* Get(FVMHandle Handle) const;
    
    bool IsValid(FVMHandle Handle) const;
...
template<typename Func>
void FVMRuntimePool::ForEachActive(Func&& Callback)
{
