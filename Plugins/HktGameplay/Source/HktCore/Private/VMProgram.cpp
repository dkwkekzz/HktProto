...
FVMProgramRegistry& FVMProgramRegistry::Get()
{
    static FVMProgramRegistry Instance;
    return Instance;
}

const FVMProgram* FVMProgramRegistry::FindProgram(const FGameplayTag& Tag) const
{
    FRWScopeLock ReadLock(Lock, SLT_ReadOnly);
    if (const TSharedPtr<FVMProgram>* Found = Programs.Find(Tag))
    {
        return Found->Get();
    }
    return nullptr;
...
void FVMProgramRegistry::RegisterProgram(FVMProgram&& Program)
{
    FRWScopeLock WriteLock(Lock, SLT_Write);
    FGameplayTag Tag = Program.Tag;
    Programs.Add(Tag, MakeShared<FVMProgram>(MoveTemp(Program)));
}

void FVMProgramRegistry::Clear()
{
    FRWScopeLock WriteLock(Lock, SLT_Write);
    Programs.Empty();
}
...
{
    FVMProgramRegistry::Get().RegisterProgram(Build());
}