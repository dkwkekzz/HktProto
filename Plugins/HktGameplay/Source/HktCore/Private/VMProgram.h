...
/**
 * FVMProgram - 컴파일된 바이트코드 프로그램 (불변, 공유 가능)
 */
struct HKTSIMULATION_API FVMProgram
{
    FGameplayTag Tag;
...
/**
 * FVMProgramRegistry - EventTag → Program 매핑 관리
 */
class HKTSIMULATION_API FVMProgramRegistry
{
public:
    static FVMProgramRegistry& Get();
    
    const FVMProgram* FindProgram(const FGameplayTag& Tag) const;
    void RegisterProgram(FVMProgram&& Program);
    void Clear();

private:
    FVMProgramRegistry() = default;
    
    TMap<FGameplayTag, TSharedPtr<FVMProgram>> Programs;
 ...
 *       .End();
 */
class HKTSIMULATION_API FFlowBuilder
{
public:
