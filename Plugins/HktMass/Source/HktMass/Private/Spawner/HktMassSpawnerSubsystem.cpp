...
#include "HktMassSpawnerSubsystem.h"
...
void UHktMassSpawnerSubsystem::Initialize(FSubsystemCollectionBase & Collection)
{
	Super::Initialize(Collection);
	Collection.InitializeDependency<UMassSpawnerSubsystem>();
}

void UHktMassSpawnerSubsystem::Deinitialize()
{
	Super::Deinitialize();
}

void UHktMassSpawnerSubsystem::SpawnEntitiesFromConfig(const FHktSpawnConfig& SpawnConfig, const FVector& CenterLocation)
{
	UMassSpawnerSubsystem* MassSpawnerSubsystem = UWorld::GetSubsystem<UMassSpawnerSubsystem>(GetWorld());
	if (!MassSpawnerSubsystem || !SpawnConfig.EntityConfig)
	{
		UE_LOG(LogTemp, Error, TEXT("Cannot spawn entities: Invalid config or subsystem"));
		return;
	}

	TArray<FVector> SpawnPositions = CalculateSpawnPositions(SpawnConfig, CenterLocation);

	const FMassEntityTemplate& EntityTemplate = SpawnConfig.EntityConfig->GetConfig().GetOrCreateEntityTemplate(*GetWorld());

	TArray<FMassEntityHandle> Entities;
	MassSpawnerSubsystem->SpawnEntities(EntityTemplate, SpawnConfig.SpawnCount, Entities);
	...
	UE_LOG(LogTemp, Log, TEXT("Spawned %d entities at location: %s"), Entities.Num(), *CenterLocation.ToString());
}

void UHktMassSpawnerSubsystem::DespawnAllEntities()
{
	UMassSpawnerSubsystem* MassSpawnerSubsystem = UWorld::GetSubsystem<UMassSpawnerSubsystem>(GetWorld());
	...
	UE_LOG(LogTemp, Log, TEXT("Despawned all entities"));
}

TArray<FVector> UHktMassSpawnerSubsystem::CalculateSpawnPositions(const FHktSpawnConfig& Config, const FVector& CenterLocation) const
{
