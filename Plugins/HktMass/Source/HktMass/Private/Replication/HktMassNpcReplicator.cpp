...
#include "HktMassNpcReplicator.h"
...
UHktMassNpcReplicator::UHktMassNpcReplicator()
{
}

void UHktMassNpcReplicator::AddRequirements(FMassEntityQuery& EntityQuery)
{
	// 복제???�요??Fragment ?�구?�항 추�?
...
void UHktMassNpcReplicator::ProcessClientReplication(
	FMassExecutionContext& Context, 
	FMassReplicationContext& ReplicationContext)
{
	// CalculateClientReplication ?�플�??�수 ?�출
	...
	UMassReplicatorBase::CalculateClientReplication<FHktReplicatedNpcAgentArrayItem>(
		...
		[](FMassExecutionContext& Context, int32 EntityIdx, FHktReplicatedNpcAgent& Agent, FMassClientHandle ClientHandle) -> FMassReplicatedAgentHandle
		{
			...
			AHktMassNpcClientBubbleInfo& BubbleInfo = RepSharedFragment.GetTypedClientBubbleInfoChecked<AHktMassNpcClientBubbleInfo>(ClientHandle);
